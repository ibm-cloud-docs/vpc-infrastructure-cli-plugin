...
This document provides a reference of the command line interface (CLI) commands available for the functionality of the {{site.data.keyword.cloud}} Virtual Private Cloud. Similar commands to execute these functions also are available as [API commands](https://{DomainName}/apidocs/vpc-on-classic){:new_window}.
...
1. Install the [IBM Cloud CLI](/docs/cli?topic=cloud-cli-ibmcloud-cli#overview){: new_window}.
...
3. Log in to IBM Cloud.
  ...
  You will be prompted to choose a region during login. VPC may not be available in all regions. Refer to [Creating VPCs in different regions](/docs/vpc-on-classic?topic=vpc-on-classic-creating-a-vpc-in-a-different-region) to see which regions are available today.
  {: important}

4. (Optional) To switch to a different region than the one you logged in to, use the `ibmcloud target -r <region_name>` command. For example, to manage VPC resources in the `us-south` region, run the following command:
...
This command targets the generation of infrastructure service, GC(Generation Classic) or NG(Next Generation). Plugin will target GC by default.

### `ibmcloud is target`

#### Target CLI for riaas GC or NG.

`ibmcloud is target [--gen GC | NG]`

**options**

- `--gen`: Generation of infrastructure service. Default is GC.
